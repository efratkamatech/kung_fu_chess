...
| WS Gateway | Holding the sockets, naming each connection, and moving opaque text between a socket and a subject; following a room's broadcasts once per room and fanning them out locally | **Everything else** — it may not import the engine, the model, the rules or the server, and a test enforces that. It cannot tell a move from a resignation, or a player from a spectator |
